    ...
    def _run(self, query: str, employee_id: str) -> str:
        ...
        try:
            ...
            specialized_prompt = (
                ...
                f"Focus ONLY on medical insurance, health coverage, claims, and medical benefits "
                f"information for employee {employee_id}. "
                f"Query: {query}. "
                f"Provide detailed medical information including medical limit (₹), "
                f"used amount (₹), available balance (₹), ESIC IP No., "
                f"dispensary/hospital details, medical card link, eligible family members, "
                f"claim procedures, and coverage details."
            )
